...
- **Relative time (whole file):** after `OUT`, append cumulative seconds from the first **exported** line in the file (`0` for the first, then `Date.parse(at) - origin` rounded to seconds). Per-target ordinals still reset per target; relative time does **not** reset per target. Message-only skips do not set the origin.
...
- UI: Dialog (same pattern as Load random demo) from Review timeline footer next to existing JSON **Export** in `scenario-builder.tsx`. Contents: ALT/HDG/SPD checkboxes, live preview of all lines, **Copy**, **Export to file** (download `.txt` via blob, mirror `downloadScenario`).
